...
```{r}
reg_pres <- occ_data(scientificName = 'Orthoptera',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
```{r}
mapa_registros_orthoptera_ggplot <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  theme_bw()
mapa_registros_orthoptera_ggplot
jpeg(filename = 'mapa_registros_orthoptera_ggplot.jpg', width = 2500, height = 1250, res = 250)
mapa_registros_orthoptera_ggplot
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_orthoptera.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_orthoptera.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_orthoptera.RDS')
saveRDS(ind_esp, 'indice_espacial_orthoptera.RDS')
```
...
write.csv(x = mc, file = 'matriz_de_comunidad_orthoptera.csv', row.names = T)
```

Exporta tu matriz de comunidad a un archivo de R, para usarla en análisis posteriores.

```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_orthoptera.RDS')
```
...
```{r}
pres_aus_sp <- crear_grafico_mosaico_de_mc(mc, tam_rotulo = 8)
pres_aus_sp
```
