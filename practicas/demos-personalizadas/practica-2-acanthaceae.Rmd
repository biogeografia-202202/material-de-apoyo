...
```{r}
reg_pres <- occ_data(scientificName = 'Acanthaceae',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
write.csv(table(reg_pres$species), 'numero_de_registros_de_presencia_de_acanthaceae_en_gbif_para_rd.csv', row.names = F)
...
```{r}
registros_presencia_acanthaceae_ggplot <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  geom_sf(data = reg_pres_sf_ok, size = 2, fill = 'red', color = 'red', alpha = 0.5) + # Registros GBIF
  theme(legend.position = "none") +
  theme_bw()
registros_presencia_acanthaceae_ggplot
jpeg('registros_presencia_acanthaceae.jpg', width = 2000, height = 1500, res = 200)
registros_presencia_acanthaceae_ggplot
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_acanthaceae.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_acanthaceae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_acanthaceae.RDS')
saveRDS(ind_esp, 'indice_espacial_acanthaceae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_acanthaceae.RDS')
```
...
```{r}
pres_aus_sp <- crear_grafico_mosaico_de_mc(mc, tam_rotulo = 6)
pres_aus_sp
```
