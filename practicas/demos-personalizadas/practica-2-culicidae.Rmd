...
write.csv(x = table(reg_pres$species),
          file = 'registros_de_presencia_de_culicidae_en_rd_gbif.csv',
          row.names = F)
```
...
```{r}
mapa_registros_presencia_culicidae_gbif_rd <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  theme_bw()
mapa_registros_presencia_culicidae_gbif_rd
jpeg(filename = 'mapa_registros_presencia_culicidae_gbif_rd.jpg', width = 2500, height = 1250, res = 250)
mapa_registros_presencia_culicidae_gbif_rd
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_culicidae.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_culicidae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_culicidae.RDS')
saveRDS(ind_esp, 'indice_espacial_culicidae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_culicidae.RDS')
```
...
```{r}
pres_aus_sp <- crear_grafico_mosaico_de_mc(mc, tam_rotulo = 7)
pres_aus_sp
```
