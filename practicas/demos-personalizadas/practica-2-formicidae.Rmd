...
```{r}
reg_pres <- occ_data(scientificName = 'Formicidae',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
```{r}
grafico_exportar <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  theme_bw()
grafico_exportar
# jpeg('mapa_registros_presencia_formicidae_en_hexagonos_h3_res_5.jpg', width = 2000, height = 1200, res = 150)
# grafico_exportar
# dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_formicidae.kml', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_formicidae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_formicidae.RDS')
saveRDS(ind_esp, 'indice_espacial_formicidae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_formicidae.RDS')
```
