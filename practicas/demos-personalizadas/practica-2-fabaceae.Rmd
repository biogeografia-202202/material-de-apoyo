...
```{r}
reg_pres <- occ_data(scientificName = 'Fabaceae',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
write.csv(x = table(reg_pres$species), file = 'registros_de_presencia_de_fabaceae_rd.csv', row.names = F)
```
...
```{r}
registros_presencia_rd_ggplot <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  geom_sf(data = reg_pres_sf_ok, size = 2, fill = 'green', color = 'green', alpha = 0.5) + # Registros GBIF
  theme(legend.position = "none") +
  theme_bw()
jpeg(filename = 'registros_presencia_rd_ggplot.jpg', width = 2500, height = 1250, res = 200)
registros_presencia_rd_ggplot
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_fabaceae.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_fabaceae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_fabaceae.RDS')
saveRDS(ind_esp, 'indice_espacial_fabaceae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_fabaceae.RDS')
```
...
```{r}
pres_aus_sp_1 <- crear_grafico_mosaico_de_mc(mc[, 1:(358/2)], tam_rotulo = 6)
pres_aus_sp_2 <- crear_grafico_mosaico_de_mc(mc[, (358/2+1):ncol(mc)], tam_rotulo = 6)
jpeg('grafico_mosaico_fabaceae_1.jpg', width = 3000, height = 2000, res = 200)
pres_aus_sp_1
dev.off()
jpeg('grafico_mosaico_fabaceae_2.jpg', width = 3000, height = 2000, res = 200)
pres_aus_sp_2
dev.off()
```
