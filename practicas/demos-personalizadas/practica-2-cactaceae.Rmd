...
```{r}
reg_pres <- occ_data(scientificName = 'Cactaceae',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
```{r}
mapa_registros_presencia_ggplot <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  theme_bw()
mapa_registros_presencia_ggplot
jpeg('mapa_registros_presencia_ggplot_cactaceae.jpg', width = 2000, height = 1000, res = 150)
mapa_registros_presencia_ggplot
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_cactaceae.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_cactaceae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_cactaceae.RDS')
saveRDS(ind_esp, 'indice_espacial_cactaceae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_cactaceae.RDS')
```
