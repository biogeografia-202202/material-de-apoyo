...
```{r}
reg_pres <- occ_data(scientificName = 'Bromeliaceae',
                   hasCoordinate = T,
                   country = 'DO',
                   limit = 100000)$data
...
write.csv(x = table(reg_pres$species), 'registros_de_presencia_por_especies_de_bromeliaceae.csv', row.names = F)
```
...
```{r}
registros_presencia_gbif_bromeliaceae_rd <- ggplot(data = rd) + # RD es la base, superpondremos capas
  ...
  theme_bw()
registros_presencia_gbif_bromeliaceae_rd
jpeg(filename = 'registros_presencia_gbif_bromeliaceae_rd.jpg', width = 2200, height = 1250, res = 200)
registros_presencia_gbif_bromeliaceae_rd
dev.off()
```
...
```{r, eval = F}
st_write(reg_pres_sf_ok %>% select(-networkKeys), 'registros_depurados_de_presencia_bromeliaceae.gpkg', delete_dsn = T)
st_write(ind_esp, 'indice_espacial_bromeliaceae.gpkg', delete_dsn = T)
```
...
```{r}
saveRDS(object = reg_pres_sf_ok, file = 'registros_depurados_de_presencia_bromeliaceae.RDS')
saveRDS(ind_esp, 'indice_espacial_bromeliaceae.RDS')
```
...
```{r}
saveRDS(object = mc, file = 'matriz_de_comunidad_bromeliaceae.RDS')
```
