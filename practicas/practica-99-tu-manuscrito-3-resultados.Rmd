...
```{r}
estimacion_riqueza_chao(colSums(mc_orig), 1)
```
...
```{r}
grupos_upgma_k6 <- readRDS('grupos_upgma_k6.RDS')
mi_fam_k6 <- mi_fam %>%
  mutate(g = grupos_upgma_k6) %>%
  mutate(g2 = ifelse(g %in% c(2,3), '2y3', ifelse(g %in% c(1,4), '1y4', '5y6'))) %>% 
  select(-g) %>% 
  group_by(g2) %>%
  summarise_all(sum) %>%
  select(-g2) %>% 
  mutate(N = nrow(mc_orig)) %>% 
  relocate(N, .before = 1) %>% 
  data.frame
mi_fam_k6
...
ChaoSpecies(data.frame(V1 = c(nrow(mc_orig), as.numeric(colSums(mc_orig)))), datatype = 'incidence_freq', k=10, conf=0.95)

nasin_raref <- iNEXT::iNEXT(
  x = data.frame(
    V1 = c(nrow(mc_orig), as.numeric(colSums(mc_orig))),
    V2 = c(nrow(mc_orig), as.numeric(colSums(mc_orig)))),
  q=0,
  knots = 400,
