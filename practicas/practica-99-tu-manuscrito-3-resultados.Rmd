...
knitr::opts_chunk$set(echo = T, cache=F, message=F, warning=F)
```
...
(m_amb_upgma_k6 <- env %>%
  ...
  st_drop_geometry() %>% 
  pivot_longer(-grupos_upgma_k6, names_to = "variable", values_to = "valor"))
(m_amb_ward_k6 <- env %>%
  select_if(is.numeric) %>%
  mutate(grupos_ward_k6) %>%
  st_drop_geometry() %>% 
  pivot_longer(-grupos_ward_k6, names_to = "variable", values_to = "valor"))
```

Evaluar efectos entre los grupos ("diferencias significativas") de los agrupamientos UPGMA y Ward. Al tratarse de 6 grupos, se utilizan las pruebas estadísticas ANOVA (evalúa homongeneidad de medias) y Kruskal-Wallis (evalúa homogeneidad de medianas).
...
m_amb_upgma_k6 %>%
  ...
  summarise(
    ...
    # p_valor_k = tryCatch(kruskal.test(valor ~ grupos_upgma_k6, exact = F)$p.value, error = function(e) NA)
...
> Te recomiendo aprender sobre pruebas estadísticas para poder interpretar mejor estos resultados. Igualmente, considera que el resultado obtenido es complicado de interpretar con certeza, debido a que determinados supuestos para ejecutar el ANOVA no se cumplen. En tal sentido, la prueba de Kruskal-Wallis, que tiene menos requisitos, surge como alternativa, pero tiene menos potencia que el ANOVA.
