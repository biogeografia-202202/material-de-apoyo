...
Crearé algunas paletas que usaré más adelante.
...
```{r}
m_amb_upgma_k6_sf <- env %>%
   rownames_to_column('hex_id') %>% 
   mutate(grupos_upgma_k6) %>%
   inner_join(za %>% select(hex_id)) %>%
  st_as_sf()
# mapa_upgma_k6 <- mapView(
#   m_amb_upgma_k6_sf %>% st_as_sf(),
#   layer.name = 'Grupos (6) UPGMA',
#   alpha.regions = 0.6,
#   map.types = 'Esri',
#   legend = T,
#   col.regions = colores_grupos,
#   zcol = 'grupos_upgma_k6') %>%
#   leaflet::setView(
#     lng = -71.5,
#     lat = 18,
#     zoom = 7)

fpal_grupos <- colorFactor(
    palette = RColorBrewer::brewer.pal(length(unique(m_amb_upgma_k6_sf$grupos_upgma_k6)), 'Set1'),
  domain = unique(m_amb_upgma_k6_sf$grupos_upgma_k6))
mapa_upgma_k6 <- leaflet(m_amb_upgma_k6_sf) %>% 
  addPolygons(
    group = ~ levels(grupos_upgma_k6),
    label = grupos_upgma_k6,
    color = ~ fpal_grupos(grupos_upgma_k6),
    stroke = F, fillOpacity = 0.7
  ) %>%
  addLegend(pal = fpal_grupos, values = ~ grupos_upgma_k6,
            group = ~ grupos_upgma_k6,
            opacity = 0.7,
            title = "Grupos UPGMA, k=6") %>%
  addTiles(group = 'OSM') %>%
  ...
  addLayersControl(
    ...
    overlayGroups = ~ levels(grupos_upgma_k6), position = 'bottomright',
